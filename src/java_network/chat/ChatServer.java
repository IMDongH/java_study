...
//추후에 제어서버와 연결할 부분
/*
 * //제어서버와 연결되는 스레드 class ControlThread extends Thread { Socket sck;
 * BufferedReader br; PrintWriter pwr;
 *
 * public ControlThread(Socket sck, PrintWriter pwr) { this.sck = sck; this.pwr
 * = pwr; try { br = new BufferedReader(new
 * InputStreamReader(sck.getInputStream())); System.out.println(sck);
 * pwr.println("relay server/1525"); System.out.println("println() 실행");
 * pwr.flush();
 *
 * } catch (IOException e) { e.printStackTrace(); } } public void run() { String
 * line = null; try { while((line = br.readLine()) != null) {
 * System.out.println("--------제어서버: "+line);
 *
 * //클라이언트로부터 quit을 받으면 종료 if(line.equals("quit")) sck.close(); } }catch
 * (Exception e) { System.out.println("제어서버와 연결 끊김..."); } } }
 */
